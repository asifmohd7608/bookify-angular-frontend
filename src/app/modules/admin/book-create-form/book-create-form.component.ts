...
import { stockValidator,availableValidator,isbnValidator } from 'src/app/shared/Validators/bookFormValidator';
...
constructor(public ufb:UntypedFormBuilder,private http:BookApiService,private router:Router){}
...
bookCreateForm=this.ufb.group({
  ...
  Book_Title:['',[Validators.required,Validators.minLength(2),Validators.maxLength(20)]],
  ...
  Language:['',[Validators.required,Validators.minLength(3),Validators.maxLength(10)]],
...
},{validator:Validators.compose([stockValidator,availableValidator,isbnValidator])})
...
onSubmit(){
  ...
  this.http.addBook(formData).subscribe((res:any)=>{
    if(!res.success){
      console.log(res)
    }else{
